...
// ServeConn runs an already-constructed Server over the given reader/writer
// (typically stdin/stdout). It blocks until the connection closes.
func ServeConn(server *Server, in io.Reader, out io.Writer) error {
...
// StdlibRoot returns the Elixir stdlib directory, or "" if not detected.
func (s *Server) StdlibRoot() string {
	return s.stdlibRoot
}

// StartGitHeadWatcher starts the background .git/HEAD poller that triggers an
// incremental reindex on branch switches. The LSP session starts this in
// Initialized; headless callers (MCP) start it explicitly.
func (s *Server) StartGitHeadWatcher() {
	s.watchGitHead()
}
...
// FileText returns the contents of filePath, preferring editor-owned open
// buffers over the on-disk contents (relevant in attached MCP mode).
func (s *Server) FileText(filePath string) (string, bool) {
	text, _, ok := s.readFileText(filePath)
	return text, ok
}

// FileLine returns line lineNum (1-based) of filePath, preferring editor-owned
// open buffers.
