...
- **Watching and runtime locations degrade instead of failing** — a directory the kernel refuses to watch (an inotify watch limit on a large tree) is now skipped instead of disabling the whole native watcher, and a workspace whose watcher is missing or partial runs a periodic reconciliation, so a daemon-only project with no editor still converges. Runtime files prefer `$XDG_RUNTIME_DIR/dexter`, then `/tmp/dexter-<uid>`, then `$TMPDIR/dexter-<uid>`; a candidate that cannot be created, is a symlink, is owned by another user, or leaves the socket path too long for `sockaddr_un` is skipped. Elixir and mix detection also searches the standard mise, asdf, and Homebrew locations and falls back to a login shell, so an editor that starts the daemon with a stripped PATH no longer disables stdlib indexing or formatting

- **A guard against indexing the wrong tree** — `init`, `reindex`, `lookup`, and `references` now refuse to run in a directory that shows no sign of being an Elixir project (no `mix.exs`, `.git`, or `.dexter`). A mistyped `dexter lookup` in the home directory used to create an empty database there and return nothing, and an accidental `dexter lsp` there would index every file the user owns. `-y`/`--yes` proceeds anyway, and is needed only once: the `.dexter` directory that first run creates is itself a marker. `dexter lsp` warns instead of refusing, because an editor is authoritative about what the user opened
...
- **`dexter lsp` is a stdio proxy onto the workspace daemon** — the session lives in the daemon and the protocol bytes are copied, not decoded and re-encoded, so the hop costs a copy. An editor's `exit` now ends that session and releases its lease instead of ending the process, which is what lets one daemon serve several editors at once
