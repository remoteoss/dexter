...
- **Go-to-definition reaches the line that declared a generated function** — a function a macro generated used to resolve to the top of its module. Dexter now reads the line from the compiled module's debug info, which is standard compiler output, so no framework is special-cased. A generator that stamps its functions with `@file {file, line}` for the declaring call sends definition and call hierarchy to that line. The line is used only when the BEAM is not older than the source and was compiled from the file being opened; otherwise the module line remains the answer. This is the Dexter side of go-to-definition for Ash code interfaces ([#108](https://github.com/remoteoss/dexter/issues/108)); Ash itself still has to stamp its generated functions with `@file`
