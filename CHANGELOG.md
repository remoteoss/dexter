...
- **A guard against indexing the wrong tree** — `init`, `reindex`, `lookup`, and `references` now refuse to run in a directory that shows no sign of being an Elixir project (no `mix.exs`, `.git`, or Dexter database). A mistyped `dexter lookup` in the home directory used to create an empty database there and return nothing, and an accidental `dexter lsp` there would index every file the user owns. `-y`/`--yes` proceeds anyway, and is needed only once: the `.dexter/dexter.db` file that first run creates is itself a marker. `dexter lsp` warns instead of refusing, because an editor is authoritative about what the user opened
...
- **`lookup`, `references`, and `reindex` go through the daemon** — they no longer open `.dexter/dexter.db` themselves, so a shell command cannot race the editor's writer and every frontend answers from one index. A burst of filesystem events is coalesced into one pass, while a lone save is indexed immediately rather than waiting out a debounce window. An empty result now says when the workspace is still building instead of looking like "no matches", and a workspace held by an offline maintenance pass such as `dexter init` is reported as exactly that as soon as the ownership lock proves no daemon can be coming, instead of waiting out the daemon start timeout. `dexter init` is the exception: it takes exclusive ownership, asking an idle daemon to exit and refusing while a client is attached

- **`lookup` and `references` answer like the editor** — the CLI now resolves names through the same semantic navigation as go-to-definition and find-references: `use` chains, aliases injected by `__using__`, generated functions from compiled BEAMs, `defdelegate` targets, and bare calls inside the defining module. A reference that appears on the same line twice is listed once. `lookup --strict` still refuses fallbacks: it never answers with a parent module's definition for a function generated into a module that has no source. `reindex` accepts a deleted file or directory and removes it from the index, and says so when a path neither exists nor has anything indexed under it, which is usually a typo

- **`dexter lsp` is a stdio proxy onto the workspace daemon** — the session lives in the daemon and the protocol bytes are copied, not decoded and re-encoded, so the hop costs a copy. Debug lines still reach the editor's log: each session sends its own as `window/logMessage`, and the daemon's log under `/tmp/dexter-<uid>` holds every session's and CLI command's. `DEXTER_DEBUG=true` takes effect when the daemon starts, and then also covers CLI commands. An editor's `exit` now ends that session and releases its lease instead of ending the process, which is what lets one daemon serve several editors at once

### Fixed

- **Git worktrees nested inside a project were indexed as part of it** — a linked worktree checked out below the project root (Claude Code's `.claude/worktrees/`, or any gitignored worktree folder) is a full copy of the repository, so every go-to-definition returned one result per checkout. The indexer, the incremental sweep, the native watcher and single-file updates now skip any directory whose `.git` file points into `.git/worktrees/`; submodules (`.git/modules/`) and Mix git dependencies stay indexed. Starting Dexter inside such a worktree no longer climbs to the enclosing checkout's `.dexter/dexter.db` either: the worktree is its own project
