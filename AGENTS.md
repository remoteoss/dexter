...
- **A workspace has to look like one**: `init`, `reindex`, `lookup`, and `references` refuse a directory with no `mix.exs`, `.git`, or Dexter database unless `-y/--yes` is passed, so a mistyped command cannot build an index over the home directory. `lsp` warns and serves anyway (the editor decides), and `stop` never checks, so a stray daemon can always be cleaned up
