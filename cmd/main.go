...
func main() {
	...
	lspCmd.Flags().StringVar(&lspMCPListen, "mcp-listen", "", "Also serve MCP over streamable HTTP at this address, sharing the LSP session")
...
func cmdLSP(projectRoot string, mcpListen string) {
	...
	serveErr := dexter_lsp.ServeConn(server, os.Stdin, os.Stdout)
